...
const Sidebar = ({ mobileOpen, onClose }) => {
  ...
  const navItems = isAdmin ? adminNav : isTeacher ? teacherNav : isParent ? parentNav : studentNav;

  const content = (
    <>
      <div className="p-6 border-b border-gray-200">
        <h1 className="text-xl font-bold text-primary-700">SchoolMS</h1>
        <p className="text-xs text-gray-500 mt-1">Phase 2</p>
      </div>
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
        {navItems.map((item) => (
          <NavLink key={item.to} to={item.to} onClick={onClose}
            className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-50'}`}>
